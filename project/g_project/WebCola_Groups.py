        ...
        for node in origin:
            ...
            if len(leaf_main) > 0:
                ...
                if node['G_name'] == leaf_main['G_name']:
                    ...
                    desc_layer = {}
                    id, nodes, links = add_main_leaf(node, nodes, links)
                    ...
                    desc_layer = {}
                    i=0
                    logger.debug(f'going through main leaf where i = {i}, node G_name = {node["G_name"]}, id = {id}')
                    i=i+1
                    leaves.append(id)
                    # add the secondary leaves
                    if len(leaf_secondary) > 0:
                        j=0
                        for leaf in leaf_secondary:
                            id2, nodes, links = add_secondary_leaf(id, leaf, nodes, links)
                            lnode = nodes[id2]
                            ...
                            description.append(desc_layer)
                            desc_layer = {}
                            logger.debug(f'going through second leaf where j = {j}, node G_name = {node["G_name"]}, id = {id}, id2 = {id2}')
                            leaves.append(id2)
                    
                    # add the results into the group_def record
                    group_type = 'group_of_leaves'
                    results.append({'leaves': leaves, 'leaf_description': description, 'group_type' : group_type})    
    ...
    for group_def in group_def_list:
        ...
        if group_def['groups'] != []:
            ...
            for nested_def in nested_groups:
                ...
                nested_def['group'] = globals()[nested_group_name]

        # process the group definitions and return results
        colaGraph, globals()[group_name]  = get_V_group(colaGraph, globals()[group_name] )    
...
def main(myquery):
    ...
    v_event_rec = {'leaves': [{'G_name': 'event'}, 
        {'G_name': 'eventName', 'role': 'has'}
    ], 'groups': [],
    'results':[],
    'ops': {'colour_list':'Yellows','level': 0, 'label': 'Event Core'}}
    colaGraph, v_event_rec  = get_V_group(colaGraph, v_event_rec )


    logger.debug('--------------------------------------------------------------------')
    logger.debug('|||||||||||||||||||||| Group Description 2 |||||||||||||||||||||||||||||')
    logger.debug('--------------------------------------------------------------------')
    


    v_event = {'leaves': [ {'G_name': 'trace'}, 
        {'G_name': 'index', 'role': 'has'}, 
        {'G_name': 'traceId', 'role': 'has'} 
    ], 'groups': [{'group': v_event_rec, 'condition': 'leaf', 'l_G_name': 'trace', 'l_role': 'item' }],
    'results':[],
    'ops': {'colour_list':'Yellows','level': 1, 'label': 'Event Record'}}
    colaGraph, v_event  = get_V_group(colaGraph, v_event)

    logger.debug('--------------------------------------------------------------------')
    logger.debug('|||||||||||||||||||||| Group Description 3s |||||||||||||||||||||||||||||')
    logger.debug('--------------------------------------------------------------------')
    


    v_trace = {'leaves': [], 
    'groups': [{'group': v_event, 'condition': 'value', 'g_G_name': 'trace', 'g_role': 'has',
                'basis': 'distinct', 'v_G_name': 'traceId'}],
    'results':[],
    'ops': {'colour_list':'Yellows','level': 2, 'label': 'Trace Record'}}
    colaGraph, v_trace  = get_V_group(colaGraph, v_trace )
    ...
    logger.debug('--------------------------------------------------------------------')
    logger.debug('|||||||||||||||||||||| Group Description 5 |||||||||||||||||||||||||||||')
    logger.debug('--------------------------------------------------------------------')


    v_att = {'leaves': [], 
    'groups': [{'group': '', 'condition': 'type' }],
    'results':[],
    'ops': {'colour_list':'Blues','level': 1, 'label': 'Group of Attributes'}}
    colaGraph, v_att  = get_V_group(colaGraph, v_att )
   

    colaGraph['basic'] = raw


    with open("groupsv2.json", "w") as outfile:  
        json.dump(colaGraph, outfile) 
